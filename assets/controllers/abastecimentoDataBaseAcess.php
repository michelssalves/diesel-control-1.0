...
function filtrarAbastecimentos($filtroPrefixo, $filtroCombustivel,$filtroMarca, $filtroModelo,$dataHoraIncial, $dataHoraFinal){
    ...
    include 'functions.php';
        ...
        $sql = $pdo->prepare("SELECT v.id_veiculo, a.bomba, a.diferencahr, a.ultimohr, a.id_abastecimento, a.data_abastecimento, v.placa, v.numero_equipamento, v.descricao_caminhao, a.odometroinicial,a.odometrofinal, a.litros_od, a.litros, 
        ...
        if ($sql->rowCount() > 0) {
            ...
            foreach($lista as $row){
                ...
                $url ='alterarAbastecimento.php?'.$info;
                ...
                <td class="w3-left-align" width="170px;"> '.($row['data_abastecimento']).'</td>
...
function registrarAbastecimento(){
    ...
    header("Location: registroDeAbastecimento.php");
...
function alterarAbastecimento(){
    ...
    header("Location: sucesso.php?acao=alterarAbastecimento");
}   
function excluirAbastecimento(){
    ...
    header("Location: sucesso.php?acao=excluir");
        ...
        if ($sql->rowCount() > 0) {
            ...
            $lista = $sql->fetchAll(PDO::FETCH_ASSOC);
             ...
             foreach($lista as $row){
                    ...
                    <td onclick="'.$link.'" style="cursor:pointer;"> '.$row['numero_equipamento'].' </td>
                    <td> '.$row['placa'].' </td>
                    <td> '.$row['prefixo'].' </td>
